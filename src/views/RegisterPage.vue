      ...
      <v-alert v-if="error" type="error" v-html="error"></v-alert>
        <v-card>
          <v-list-item tow-line>
            <v-list-item-content>
              <v-list-item-title class="headline mb-1">Register Form</v-list-item-title>
              <v-list-item-subtitle>register to view users and resources</v-list-item-subtitle>
            </v-list-item-content>
          </v-list-item>
          <v-divider></v-divider>
            ...
            <v-col cols="10">
              <v-form v-model="valid" @submit.prevent="register">
                ...
                <v-text-field
                  ...
                  required
                ></v-text-field>
                <v-btn text type="submit" :loading="loading">Register</v-btn>
              </v-form>
            </v-col>
          </v-row>
        </v-card>
  ...
  methods: {
    register() {
      if(this.valid) {
        ...
        this.$store.dispatch('user/setRegisterError')
      }
    }
  },
  created() {
